...
import app.modules.rdkitmodules
import app.modules.cdkmodules
...
def getMolculeHash(smiles:str):
    ...
    if mol:
        ...
        Canonical_SMILES = Chem.MolToSmiles(mol,kekuleSmiles=True,isomericSmiles=False)
        return {
            'Formula': Formula,
...
def COCONUTpreprocessing(input_text:str):
    ...
    standardised_SMILES = Chem.MolToSmiles(Chem.MolFromMolBlock(standarised_mol_block),kekuleSmiles=True)
    molecule_hash = getMolculeHash(standardised_SMILES)
    parent_canonical_smiles = molecule_hash['Canonical_SMILES']
    parent_2D_molblock = cdkmodules.getCDKSDGMol(parent_canonical_smiles,V3000=False).replace("$$$$\n", "")
    parent_3D_molblock = rdkitmodules.get3Dconformers(parent_canonical_smiles)
    parent_2D_molblock_v3 = cdkmodules.getCDKSDGMol(parent_canonical_smiles,V3000=True).replace("$$$$\n", "")
    ...
    if rdkitmodules.has_stereochemistry(standardised_SMILES):
        ...
        variant_2D_molblock = cdkmodules.getCDKSDGMol(variant_isomeric_smiles,V3000=False).replace("$$$$\n", "")
        ...
        return {'original_mol': original_mol,
