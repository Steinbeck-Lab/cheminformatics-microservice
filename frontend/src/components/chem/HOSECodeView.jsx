  ...
  return (
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-lg">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-blue-400 mb-4">HOSE Code Generator</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            {/* Spheres Selection */}
            <div>
              <label htmlFor="spheres-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Number of Spheres
              </label>
              <select
                id="spheres-select"
            ...
            {/* Toolkit Selection */}
            <div>
              <label htmlFor="toolkit-select-hose" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Toolkit
              </label>
              <select
            ...
            <div className="flex items-end pb-1">
              <div className="flex items-center">
                ...
                />
                <label htmlFor="ringsize-checkbox" className="ml-3 block text-sm text-gray-700 dark:text-gray-300">
                  Include ring size
                </label>
              </div>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-lg">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3 border-b border-gray-200 dark:border-gray-700 pb-3">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              HOSE Codes <span className="text-sm font-normal text-gray-500 dark:text-gray-400">({hoseCodes.length} atoms)</span>
            </h3>
            {/* Copy Button */}
            <button
          ...
          <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700 shadow-sm overflow-y-auto max-h-96">
            <table className="w-full border-collapse">
              ...
              <thead className="sticky top-0 bg-gray-100 dark:bg-gray-700 z-10">
                {/* Added sticky header */}
                <tr>
                  <th scope="col" className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-600">Atom Index</th>
                  <th scope="col" className="px-4 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-600">HOSE Code</th>
                </tr>
              </thead>
              {/* Table Body */}
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {hoseCodes.map((code, index) => (
                  <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-150">
                    <td className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">{index}</td>
                    <td className="px-4 py-2 font-mono text-xs text-gray-700 dark:text-gray-300 break-all">{code}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-4 text-sm text-gray-600 dark:text-gray-400">
            <p>
              HOSE codes describe the chemical environment around each atom.
              Spheres indicate distance from the central atom.
            </p>
          </div>
        </div>
      )}
      ...
      {!hoseCodes.length && !loading && !error && (
        <div className="bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 border border-blue-200 dark:border-blue-800 rounded-lg p-6 shadow" role="complementary">
          <h4 className="text-lg font-medium text-blue-800 dark:text-blue-300 mb-2">About HOSE Codes</h4>
          <p className="text-gray-700 dark:text-gray-300">
            HOSE (Hierarchically Ordered Spherical Environment) codes describe the chemical environment around each atom in a molecule.
            They are useful in NMR shift prediction, QSAR/QSPR studies, and structural elucidation.
          </p>
          <p className="mt-2 text-gray-700 dark:text-gray-300">
            The "spheres" parameter controls the depth of the environment considered (distance in bonds). More spheres yield more specific codes.
          </p>
        </div>
      )}
    </div>
