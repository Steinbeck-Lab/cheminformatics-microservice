...
const Depict3DView = ({ isActive = true }) => { // isActive prop might control if viewer should mount/render
  ...
  const destroyViewer = useCallback(() => {
    if (viewerRef.current) {
      ...
      } catch (e) { console.warn("Error cleaning up viewer:", e); }
      finally {
        viewerRef.current = null; modelRef.current = null; setViewerInitialized(false);
      }
    }
  }, []);
  ...
  const createViewer = useCallback(() => {
    ...
    try {
      const bgColorHex = BACKGROUND_COLORS.find(c => c.id === backgroundColor)?.value || '#000000';
      destroyViewer();
      const viewer = window.$3Dmol.createViewer(
        containerRef.current,
        { backgroundColor: bgColorHex, width: '100%', height: '100%', antialias: true, defaultcolors: window.$3Dmol.elementColors.rasmol }
      );
  ...
  const renderMolecule = useCallback(() => {
    ...
    const viewer = viewerRef.current;
    try {
      viewer.removeAllModels(); modelRef.current = null;
      if (typeof molData !== 'string' || !molData.includes('M  END')) throw new Error('Invalid Molblock data format.');
  ...
  const initializeViewer = useCallback(() => {
    ...
    if (viewer) {
      viewerRef.current = viewer; setViewerInitialized(true);
      if (molData) setTimeout(() => renderMolecule(), 100);
      return true;
    } else {
      setViewerInitialized(false); return false;
    }
  ...
  useEffect(() => {
    if (isActive && !viewerMounted && containerRef.current) {
      mountTimerRef.current = setTimeout(() => { setViewerMounted(true); initializeViewer(); }, 150);
    }
  ...
  useEffect(() => {
    if (molData && viewerInitialized) renderMolecule();
      ...
      viewerRef.current.removeAllModels(); modelRef.current = null; viewerRef.current.render();
    }
  ...
  useEffect(() => {
    if (viewerRef.current && viewerInitialized) {
      ...
      } catch (e) { console.warn("Error changing background color:", e); }
    }
  }, [backgroundColor, viewerInitialized]);


  // Effect for component unmount cleanup
  useEffect(() => { return () => { destroyViewer(); }; }, [destroyViewer]);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (viewerRef.current && containerRef.current && viewerInitialized) {
        try { viewerRef.current.resize(); viewerRef.current.render(); }
        catch (e) { console.warn("Error handling resize:", e); }
      }
    };
  ...
  const handleTakeScreenshot = () => {
    if (!viewerRef.current || !viewerInitialized) { setError("Viewer not ready."); return; }
    try {
      const pngDataUri = viewerRef.current.pngURI();
      if (!pngDataUri) throw new Error("Failed to generate PNG data URI.");
      const a = document.createElement('a'); a.href = pngDataUri; a.download = 'molecule-3d-screenshot.png';
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
    } catch (err) { console.error("Error taking screenshot:", err); setError("Failed to take screenshot."); }
  };

  // Handler to reset the camera view
  const handleResetView = () => {
    if (!viewerRef.current || !viewerInitialized) return;
    try { viewerRef.current.zoomTo(); viewerRef.current.render(); }
    catch (e) { console.warn("Error resetting view:", e); }
  };

  // Handler to attempt reinitialization if viewer failed
  const handleReinitialize = () => {
    setError(null); setViewerMounted(false); destroyViewer();
    setTimeout(() => { setViewerMounted(true); }, 100);
  };

  // Handle copying the molblock to clipboard
  const handleCopy = () => {
    if (!molData || !navigator.clipboard) return;
    navigator.clipboard.writeText(molData)
      .then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); })
      .catch(err => { console.error('Failed to copy 3D molblock:', err); setError('Failed to copy 3D Molblock.'); });
  };
  ...
  const downloadMolblock = () => {
    ...
    } catch (err) { console.error("Error creating download link:", err); setError("Could not create file."); }
  };
  ...
  return (
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-lg border border-gray-200 dark:border-gray-700">
        {/* Title */}
        <h2 className="text-xl font-semibold text-gray-800 dark:text-blue-400 mb-4">3D Coordinate Generation & Visualization</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {/* Toolkit Selection */}
            <div>
              <label htmlFor="toolkit-select-3d" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            {/* Background Color */}
            <div>
              <label htmlFor="bg-color-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Background Color
              </label>
              <select
                id="bg-color-select"
            ...
            {/* Color Scheme */}
            <div>
              <label htmlFor="color-scheme-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Color Scheme
              </label>
              <select
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 pt-2">
            <div className="flex items-center">
              ...
              />
              <label htmlFor="show-labels" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
          ...
          <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-white font-medium flex items-center justify-center transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 ${!smiles.trim() || loading
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium flex items-center transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800 focus:ring-indigo-500 ${loading || !molData || !viewerInitialized
            ...
            >
              <HiOutlineColorSwatch className="mr-2 h-5 w-5" /> {/* Using ColorSwatch as placeholder for reset */}
              Reset View
            </button>
          </div>

          {/* Use Example Button */}
          {!smiles && !molData && ( // Show only if input is empty and no data loaded
          ...
          {error && !loading && (
            <div className="p-4 bg-red-50 dark:bg-red-900 dark:bg-opacity-30 text-red-700 dark:text-red-200 border border-red-300 dark:border-red-700 rounded-lg mt-4 shadow-sm flex justify-between items-start" role="alert">
    ...
    {molData && !loading && !error ? (
      ...
      <div className="bg-blue-50 dark:bg-blue-900 dark:bg-opacity-20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-sm shadow">
        <div className="flex items-start space-x-3">
          <HiOutlineViewGrid className="h-5 w-5 text-blue-600 dark:text-blue-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
          <div>
            <h4 className="font-semibold text-blue-800 dark:text-blue-300">Interacting with the 3D View</h4>
            <ul className="mt-2 space-y-1 list-disc list-inside text-gray-700 dark:text-gray-300">
              <li><strong>Rotate:</strong> Click & drag</li>
              <li><strong>Zoom:</strong> Scroll wheel / Pinch gesture</li>
              <li><strong>Pan:</strong> Right-click drag / Two-finger drag</li>
              <li><strong>Reset View:</strong> Double-click / Reset View button</li>
            </ul>
          </div>
        </div>
      </div>
