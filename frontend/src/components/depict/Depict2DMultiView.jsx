  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < processingList.length; i++) {
        ...
        const title = spaceIndex > 0 ? smilesLine.substring(spaceIndex + 1).trim() : `Molecule ${i + 1}`;
  ...
  const downloadAllDepictions = async () => {
    ...
    try {
      ...
      const fetchPromises = depictions.map(async (depiction) => {
        const safeTitle = depiction.title.replace(/[^a-z0-9]/gi, '_').toLowerCase() || `molecule_${depiction.id}`;
        ...
        try {
          ...
          zip.file(`${filename}.error.txt`, `Failed to download image for ${depiction.smiles}. Error: ${fetchErr.message}`);
        }
      });
  ...
  return (
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-lg border border-gray-200 dark:border-gray-700">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-blue-400 mb-4">Batch 2D Depiction</h2>

        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Input Text Area */}
          <div>
            <div className="flex justify-between items-center mb-1">
              <label htmlFor="smiles-input-batch" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <AnimatePresence>
            {showToolsSection && (
              ...
              >
                <h3 className="text-md font-medium text-gray-700 dark:text-gray-300">Depiction Options</h3>
                {/* Tools Grid */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Toolkit Select */}
                  <div>
                    <label htmlFor="toolkit-select-batch" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                      Toolkit
                    </label>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      value={width}
                      onChange={(e) => setWidth(Math.max(50, Math.min(2000, Number(e.target.value))))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      value={height}
                      onChange={(e) => setHeight(Math.max(50, Math.min(2000, Number(e.target.value))))}
                      min="50" max="2000" step="10"
                      className="w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 shadow-sm"
                    />
                  </div>
                ...
                {/* Highlight Input */}
                <div>
                  <label htmlFor="highlight-input" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 pt-2">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className={`h-4 w-4 rounded border-gray-300 dark:border-gray-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-blue-500 dark:focus:ring-offset-gray-800 ${toolkit === 'rdkit'
                  ...
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor="unicolor" className="ml-2 text-sm text-gray-700 dark:text-gray-300">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              ...
              // Button Styling
              className={`px-5 py-2 rounded-lg text-white font-medium flex items-center justify-center transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 ${!inputText.trim() || loading
            ...
            {depictions.length > 0 && !loading && (
              <>
                ...
                <button
                  ...
                  className={`px-4 py-2 font-medium rounded-md flex items-center text-sm transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800 focus:ring-indigo-500 ${copiedSmiles
            ...
            {depictions.map((depiction) => (
              ...
              >
                ...
                <div className="p-2 flex-grow">
                  ...
                  <div className="bg-white rounded-md p-2 flex items-center justify-center h-48">
                    <img
                      ...
                      onError={(e) => {
                        console.error(`Error loading image for ${depiction.title}: ${depiction.imageUrl}`);
                        e.target.onerror = null;
                        // Use neutral fallback SVG
                        e.target.src = 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMDAgMTAwIj48dGV4dCB4PSI1MCIgeT0iNTAiIGRvbWluYW50LWJhc2VsaW5lPSJtaWRkbGUiIHRleHQtYW5jaG9yPSJtaWRkbGUiIGZvbnQtc2l6ZT0iMTAiIGZpbGw9IiM4ODg4ODg4Ij5FcnJvciBsb2FkaW5nPC90ZXh0Pjwvc3ZnPg==';
                ...
                <div className="px-3 py-2 border-t border-gray-200 dark:border-gray-700">
                  <h4 className="font-medium text-gray-800 dark:text-gray-100 text-sm truncate" title={depiction.title}>
                    {depiction.title}
                  </h4>
                  <p className="text-gray-500 dark:text-gray-400 text-xs truncate font-mono" title={depiction.smiles}>
                    {depiction.smiles}
                  </p>
                </div>
                ...
                <div className="px-3 py-2 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
                  <label htmlFor={`rotate-${depiction.id}`} className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                  ...
                  >
                    ...
                    <AnimatePresence mode='wait'>
                      {copiedSingle === depiction.id ? (
                        <motion.div
                          key="check" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} transition={{ duration: 0.15 }}
                        ...
                        <motion.div
                          key="clipboard" initial={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} transition={{ duration: 0.1 }}
      ...
      {!depictions.length && !loading && !error && (
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md dark:shadow-lg flex flex-col items-center justify-center text-center min-h-[300px] border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Enter SMILES strings (one per line, optionally with titles) and click "Generate Depictions".
          </p>
          <button
            type="button"
            onClick={generateExamples}
