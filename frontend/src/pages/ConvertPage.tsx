/**
 * ConvertPage -- tabbed interface for format conversion and coordinate generation.
 *
...
const ConvertPage = () => {
  ...
  const [activeTabId, setActiveTabId] = useState(convertId || tabs[0].id);
  ...
  useEffect(() => {
    if (convertId) {
      const tab = tabs.find((t) => t.id === convertId);
      if (tab) {
        setActiveTabId(convertId);
      } else {
  ...
  const handleTabSelection = (tabId: string) => {
    ...
    navigate(`/convert/${tabId}`, { replace: true });
  };
        ...
        <div className="w-full lg:w-3/4 mx-auto">
          ...
          >
            ...
            {activeTab && (
              ...
              >
                <div className="bg-linear-to-r from-teal-500 to-cyan-600 dark:from-teal-700 dark:to-cyan-900 rounded-xl shadow-xl overflow-hidden relative">
                  ...
                  <div className="relative p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
                    <div className="space-y-1 text-center md:text-left max-w-3xl">
                      ...
                      <p className="text-teal-50 dark:text-cyan-100 text-xs md:text-base opacity-90">
