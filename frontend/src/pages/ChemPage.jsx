...
const tabs = [
    // Structure
    { id: 'stereoisomers', name: 'Stereoisomers', description: 'Generate all possible stereoisomers', icon: HiOutlineDocumentDuplicate, category: 'structure', component: StereoisomersView },
    { id: 'hosecodes', name: 'HOSE Codes', description: 'Generate HOSE codes', icon: HiOutlineCode, category: 'structure', component: HOSECodeView },
    { id: 'standardize', name: 'Standardize', description: 'Standardize structures', icon: HiOutlineRefresh, category: 'structure', component: StandardizeView },
    { id: 'functional-groups', name: 'Functional Groups', description: 'Identify functional groups', icon: HiOutlineCollection, category: 'structure', component: ErtlFunctionalGroupView },
    { id: 'tautomer', name: 'Tautomers', description: 'Generate standardized tautomers', icon: HiOutlineRefresh, category: 'structure', component: StandardizedTautomerView },
    // Analysis
    { id: 'descriptors', name: 'Descriptors', description: 'Calculate molecular descriptors', icon: HiOutlineChartSquareBar, category: 'analysis', component: DescriptorsView },
    ...
    // Advanced
    { id: 'coconut', name: 'COCONUT Preprocessing', description: 'Prepare data for COCONUT DB', icon: HiOutlineDatabase, category: 'advanced', component: CoconutPreProcessingView },
    { id: 'classyfire', name: 'ClassyFire', description: 'Chemical classification', icon: HiOutlineTag, category: 'advanced', component: ClassyfireView },
];
    ...
    return (
        ...
        >
            ...
            <motion.button
                ...
                onClick={handleSidebarToggle} aria-label={sidebarOpen ? "Close sidebar" : "Open sidebar"} whileTap={{ scale: 0.95 }}
            >
                <AnimatePresence mode="wait" initial={false}>
                    {sidebarOpen ? (<motion.div key="close-sb" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.2 }}> <HiOutlineX className="h-6 w-6" /> </motion.div>) : (<motion.div key="open-sb" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.2 }}> <HiOutlineMenuAlt1 className="h-6 w-6" /> </motion.div>)}
                </AnimatePresence>
            </motion.button>
            ...
            <div className="relative w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 z-10">
                <div className="w-full lg:w-3/4 mx-auto">
                    <motion.div className="mb-8 md:mb-10 text-center mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                        <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[var(--text-primary)] mb-3">Chemical Analysis Tools</h1>
                    ...
                    <div className="flex flex-col md:flex-row md:gap-x-4 lg:gap-x-6">
                        ...
                        fixed inset-y-0 left-0 z-40 w-64 mt-16 md:mt-0 md:sticky md:top-20 md:h-[calc(100vh-10rem)] transform transition-transform duration-300 ease-in-out md:transform-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                                }`}
                        >
                            <div className="h-full overflow-y-auto">
                                ...
                                <div className="sticky top-0 z-10 p-2 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700/50">
                                    <LayoutGroup id="viewModeToggle-chem">
                                        <div className="flex p-1 bg-slate-200 dark:bg-slate-900 rounded-lg">
                                            <motion.button className={`relative flex-1 py-1.5 px-3 rounded-md font-medium text-xs sm:text-sm transition-colors ${viewMode === 'categories' ? 'text-sky-800 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`} onClick={() => setViewMode('categories')} >
                                                {viewMode === 'categories' && <motion.div layoutId="viewModePill-chem" className="absolute inset-0 bg-white dark:bg-slate-700 rounded-md shadow-sm" transition={{ type: 'spring', stiffness: 400, damping: 35 }} />}
                                                <span className="relative z-10 flex items-center justify-center gap-1"><HiOutlineTemplate className="h-4 w-4" /> Categories</span>
                                            </motion.button>
                                            <motion.button className={`relative flex-1 py-1.5 px-3 rounded-md font-medium text-xs sm:text-sm transition-colors ${viewMode === 'all' ? 'text-sky-800 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`} onClick={() => setViewMode('all')} >
                                                {viewMode === 'all' && <motion.div layoutId="viewModePill-chem" className="absolute inset-0 bg-white dark:bg-slate-700 rounded-md shadow-sm" transition={{ type: 'spring', stiffness: 400, damping: 35 }} />}
                                                <span className="relative z-10 flex items-center justify-center gap-1"><HiOutlineViewList className="h-4 w-4" /> All Tools</span>
                                            </motion.button>
                                        </div>
                                    </LayoutGroup>
                                </div>
                                ...
                                {/* Navigation List */}
                                <motion.div className="p-3" variants={sidebarStaggerContainer} initial="hidden" animate="visible" >
                                    <LayoutGroup id="toolNav-chem">
                                        {viewMode === 'categories' ? (
                                            <div className="space-y-3">
                                                {categoryOrder.map(category => (
                                                    tabsByCategory[category] && (
                                                        <motion.div key={category} variants={sidebarItemVariant} className="border-b border-slate-300/70 dark:border-slate-700/50 pb-3 last:border-b-0">
                                                            {/* Removed category toggle button */}
                                                            <h3 className={`text-xs font-semibold px-2 py-1 ${getCategoryColorClass(category, 'text')} uppercase tracking-wider mb-1`}> {categories[category]?.name || category} </h3>
                                                            {/* Always show items, removed AnimatePresence & variants */}
                                                            <div className="mt-2 space-y-1">
                                                                {tabsByCategory[category].map(tab => {
                                                                    const isActive = activeTabId === tab.id;
                                                                    return (<button key={tab.id} onClick={() => handleTabClick(tab.id)} className={`relative w-full flex items-center px-3 py-2 text-sm rounded-md transition-colors group ${isActive ? 'text-sky-700 dark:text-white' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/40 dark:hover:bg-slate-700/40'}`} > {isActive && (<motion.div className="absolute inset-0 bg-sky-100 dark:bg-slate-700 rounded-md -z-10" layoutId="activeToolIndicator-chem" transition={{ type: 'spring', stiffness: 350, damping: 30 }} />)} <tab.icon className={`h-5 w-5 mr-2 flex-shrink-0 ${isActive ? getCategoryColorClass(category, 'text') : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} /> <span className="truncate">{tab.name}</span> </button>);
                                                                })}
                                                            </div>
                                                        </motion.div>
                                                    )
                                            ...
                                            <div className="space-y-1">
                                                {tabs.map(tab => {
                                                    const isActive = activeTabId === tab.id;
                                                    return (<motion.button key={tab.id} variants={sidebarItemVariant} onClick={() => handleTabClick(tab.id)} className={`relative w-full flex items-center px-3 py-2 text-sm rounded-md transition-colors group ${isActive ? 'text-sky-700 dark:text-white' : 'text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/40 dark:hover:bg-slate-700/40'}`} > {isActive && (<motion.div className="absolute inset-0 bg-sky-100 dark:bg-slate-700 rounded-md -z-10" layoutId="activeToolIndicator-chem" transition={{ type: 'spring', stiffness: 350, damping: 30 }} />)} <tab.icon className={`h-5 w-5 mr-2 flex-shrink-0 ${isActive ? getCategoryColorClass(tab.category, 'text') : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} /> <span className="truncate">{tab.name}</span> </motion.button>);
                                                })}
                                            </div>
                                        )}
                                    </LayoutGroup>
                                    ...
                                    {activeTab && (
                                        <div className="p-5 sm:p-6 border-b border-slate-200/80 dark:border-slate-700/50 bg-white/30 dark:bg-slate-800/20">
                                            <div className="flex items-start sm:items-center">
                                                <activeTab.icon className={`h-6 w-6 sm:h-7 sm:w-7 mr-3 sm:mr-4 flex-shrink-0 mt-0.5 sm:mt-0 ${getCategoryColorClass(activeTab.category, 'text')}`} />
                                                <div>
                                                    <h1 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">{activeTab.name}</h1>
                                                    <p className="text-sm text-[var(--text-secondary)] mt-1">{activeTab.description}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                        ...
                        </div> {/* End Main Content Flex Item */}
