...
import {
  HiOutlineArrowRight, HiOutlineBeaker, HiOutlineArrowsRightLeft,
  HiOutlineEye, HiOutlineWrenchScrewdriver, HiOutlineCamera,
  HiOutlineCubeTransparent, HiOutlineCodeBracketSquare
...
const features = [
  { title: 'Chemical Analysis', description: 'Analyze molecules with descriptors, stereoisomer generation, and similarity calculations.', icon: HiOutlineBeaker, link: '/chem' },
  { title: 'Format Conversion', description: 'Convert between different chemical file formats (SMILES, InChI, 2D/3D coordinates).', icon: HiOutlineArrowsRightLeft, link: '/convert' },
  { title: 'Visualization', description: 'Generate customizable 2D and 3D visualizations of chemical structures.', icon: HiOutlineEye, link: '/depict' },
  { title: 'Structure Tools', description: 'Specialized tools for structure generation, sugar removal, and more.', icon: HiOutlineWrenchScrewdriver, link: '/tools' },
  { title: 'OCSR', description: 'Optical Chemical Structure Recognition to extract structures from images.', icon: HiOutlineCamera, link: '/ocsr' }
];
...
const TiltCard = ({ children, className = "", tiltIntensity = 7 }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.035, transition: { type: 'spring', stiffness: 200, damping: 20 } }}
    >
      {children}
    </motion.div>
  );
    ...
    <div className="relative min-h-screen w-full bg-slate-100 dark:bg-gray-950 text-slate-900 dark:text-slate-100 font-sans overflow-x-hidden isolate">
      ...
      <div className="relative w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 z-10">
        ...
        <div className="w-full lg:w-3/4 mx-auto">
          ...
          >
            {/* ... Glows ... */}
            <motion.div className="absolute -top-20 -left-20 w-80 h-80 sm:w-96 sm:h-96 bg-gradient-radial from-sky-400/20 via-transparent dark:from-blue-600/20 dark:via-transparent to-transparent rounded-full blur-3xl opacity-80 dark:opacity-60 pointer-events-none" style={{ x: '-10%', y: '-10%' }} whileInView={{ x: '0%', y: '0%', scale: [1, 1.05, 1], transition: { duration: 1.8, ease: "easeInOut" } }} viewport={{ amount: 0.3 }} />
            <motion.div className="absolute -bottom-20 -right-20 w-72 h-72 sm:w-80 sm:h-80 bg-gradient-radial from-indigo-400/20 via-transparent dark:from-cyan-600/20 dark:via-transparent to-transparent rounded-full blur-3xl opacity-70 dark:opacity-50 pointer-events-none" style={{ x: '10%', y: '10%' }} whileInView={{ x: '0%', y: '0%', scale: [1, 1.03, 1], transition: { duration: 1.8, ease: "easeInOut", delay: 0.2 } }} viewport={{ amount: 0.3 }} />
            ...
            {/* Sub-headline */}
            <motion.p variants={heroTextVariants} custom={2} className="text-base sm:text-lg md:text-xl text-slate-600 dark:text-slate-300 max-w-4xl mx-auto mb-10 sm:mb-12">
              Visualize, analyze, and manipulate chemical structures seamlessly using the powerful Cheminformatics Microservice API.
            </motion.p>

            {/* Button Container */}
            <motion.div variants={heroTextVariants} custom={3} className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4 md:gap-6">
              {/* Primary Button */}
              <Link to="/depict" className="group primary-button w-full sm:w-auto relative inline-flex items-center justify-center px-7 py-3 sm:px-9 sm:py-4 text-base sm:text-lg font-semibold text-white bg-gradient-to-r from-sky-600 to-cyan-500 dark:from-blue-600 dark:to-cyan-500 rounded-xl shadow-lg hover:shadow-xl hover:brightness-110 dark:hover:shadow-cyan-500/30 transition-all duration-300 ease-out transform hover:scale-[1.06] focus:outline-none focus:ring-4 ring-offset-2 ring-offset-slate-100 dark:ring-offset-gray-950 focus:ring-cyan-500/50 dark:focus:ring-cyan-400/50">
              ...
              <a href="https://api.naturalproducts.net/latest/docs" target="_blank" rel="noopener noreferrer" className="group w-full sm:w-auto relative inline-flex items-center justify-center px-7 py-3 sm:px-9 sm:py-4 text-base sm:text-lg font-semibold text-slate-700 dark:text-slate-200 bg-white/90 dark:bg-slate-800/80 backdrop-blur-md border border-slate-300 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700/90 hover:border-slate-400 dark:hover:border-slate-500 rounded-xl shadow-md hover:shadow-lg dark:hover:shadow-slate-700/40 transition-all duration-300 ease-out transform hover:scale-[1.06] focus:outline-none focus:ring-4 focus:ring-slate-400/50 dark:focus:ring-slate-600/50 ring-offset-2 ring-offset-slate-100 dark:ring-offset-gray-950">
                ...
                <span className="relative z-10 flex items-center"><HiOutlineCodeBracketSquare className="mr-2 h-5 w-5 text-sky-600 dark:text-sky-400" /> API Docs</span>
              </a>
            </motion.div>
          </motion.section>
          ...
          >
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-12 md:mb-16 tracking-tight text-slate-900 dark:text-white">
              Core
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-600 to-indigo-600 dark:from-blue-400 dark:to-cyan-400"> Capabilities</span>
            </h2>
                ...
                return (
                  <motion.div key={index} variants={itemVariants}>
                    <TiltCard tiltIntensity={7}>
                      <Link to={feature.link} className="feature-card-enhanced group relative flex flex-col h-full min-h-[270px] sm:min-h-[290px] bg-white/80 dark:bg-slate-800/75 backdrop-blur-xl border border-transparent rounded-3xl p-6 sm:p-8 transition-all duration-400 ease-out shadow-lg dark:shadow-xl hover:shadow-xl dark:hover:shadow-blue-900/30 overflow-hidden transform-style-3d" style={{ transform: 'translateZ(0)', willChange: 'border-color, backdrop-filter' }}>
                        ...
                        <div style={{ transform: 'translateZ(25px)' }}>
                          <h3 className="text-lg sm:text-xl font-semibold text-slate-800 dark:text-white mb-2 sm:mb-3">{feature.title}</h3>
                          <p className="text-sm sm:text-[15px] text-slate-600 dark:text-slate-300 flex-grow mb-5 sm:mb-6 leading-relaxed">{feature.description}</p>
                        </div>
                        <div className="mt-auto flex items-center text-sky-700 dark:text-sky-400 group-hover:text-cyan-800 dark:group-hover:text-cyan-200 font-medium transition-colors duration-300 text-sm" style={{ transform: 'translateZ(15px)' }}>
          ...
          {recentMolecules && recentMolecules.length > 0 && (
            ...
            >
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 sm:mb-12 gap-3 sm:gap-6">
                ...
                <Link to="/chem" className="text-sky-700 dark:text-sky-400 hover:text-indigo-700 dark:hover:text-cyan-200 flex-shrink-0 flex items-center font-medium transition-colors duration-300 group text-sm hover-underline-link">
                  <span>View all</span>
                  ...
                  <motion.div key={index} variants={itemVariants}>
                    <TiltCard tiltIntensity={5}>
                      <div className="relative bg-white/80 dark:bg-slate-800/70 backdrop-blur-lg sm:backdrop-blur-xl rounded-2xl shadow-lg dark:shadow-xl border border-slate-200 dark:border-slate-700/80 overflow-hidden transition-all duration-300 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/80 p-1">
                        ...
                        <MoleculeCard smiles={molecule.smiles} title={molecule.name || `Molecule ${index + 1}`} size="sm" className="p-4 text-slate-800 dark:text-white bg-transparent" />
                      </div>
                    </TiltCard>
                  </motion.div>
                ))}
        ...
        >
          ...
          <div className="max-w-screen-lg mx-auto flex flex-col lg:flex-row items-center text-center lg:text-left px-4 sm:px-6 lg:px-8">
            <motion.div className="mb-8 sm:mb-10 lg:mb-0 lg:mr-16 flex-shrink-0 text-sky-600 dark:text-blue-500/90" variants={ribbonContentVariants} custom={0} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.5 }} animate={{ rotate: 360 }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }}>
              <HiOutlineCubeTransparent className="h-28 w-28 sm:h-32 sm:w-32 md:h-40 md:w-40 mx-auto lg:mx-0 opacity-90 drop-shadow-lg" />
            </motion.div>
            <motion.div className="flex-grow lg:max-w-3xl" variants={ribbonContentVariants} custom={1} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }}>
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4 sm:mb-5 tracking-tight">Designed for Chemists. Fueled by Open-Source.</h3>
              <p className="text-base md:text-lg text-slate-700 dark:text-slate-300 leading-relaxed">
                ...
                <a href="https://cheminf.uni-jena.de" target="_blank" rel="noopener noreferrer" className="text-sky-700 dark:text-sky-300 hover:text-indigo-700 dark:hover:text-cyan-300 font-medium transition-colors duration-300 hover-underline-link">Steinbeck Lab</a>
                {' '}at the{' '}
                <a href="https://www.uni-jena.de/en" target="_blank" rel="noopener noreferrer" className="text-sky-700 dark:text-sky-300 hover:text-indigo-700 dark:hover:text-cyan-300 font-medium transition-colors duration-300 hover-underline-link">Friedrich Schiller University Jena</a>.
              </p>
            </motion.div>
          </div>
        </motion.section>
