...
const tabs = [
  {
    ...
    icon: HiOutlineCube, // Consider a more specific icon if available, e.g., related to molecules/cutting
    description: 'Remove sugar moieties from complex molecular structures'
  },
  {
...
const contentVariants = {
  hidden: { opacity: 0, x: -15 }, // Slide effect
        ...
        <div className="w-full lg:w-3/4 mx-auto">
          ...
          >
            ...
            <motion.p variants={headerItemVariants} className="text-[var(--text-secondary)] text-base md:text-lg max-w-3xl mx-auto">
              Specialized tools for structure generation, sugar moiety removal, and more.
            </motion.p>
          </motion.div>
          ...
          >
            ...
            <div className="relative border-b border-slate-200/80 dark:border-slate-700/50 bg-slate-100/50 dark:bg-slate-800/30">
              <div className="flex justify-center overflow-x-auto p-2 space-x-1">
                <LayoutGroup id="tools-tabs"> {/* Unique layoutId */}
                  {tabs.map((tab) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`tab-button relative flex items-center px-4 py-2.5 text-sm rounded-lg transition-colors whitespace-nowrap focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-secondary)] focus-visible:ring-[var(--text-accent)] ${isActive
                      ...
                      >
                        ...
                        {isActive && (
                          <motion.div
                            ...
                            layoutId="activeTabPill-tools" // Unique layoutId
                        ...
                        >
                          <tab.icon className={`h-5 w-5 mr-2 flex-shrink-0 transition-colors duration-200 ${isActive ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
                          <span>{tab.name}</span>
                        </motion.span>
                      </motion.button>
                    );
            ...
            {activeTab && (
              ...
              >
                <div className="flex items-start sm:items-center max-w-5xl mx-auto">
                  ...
                  <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)]">{activeTab.name}</h2>
