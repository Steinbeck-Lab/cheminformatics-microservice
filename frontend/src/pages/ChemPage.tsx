/**
 * ChemPage -- sidebar + tab navigation for 14 chemical analysis tools.
 *
...
const ChemPage = () => {
  ...
  const handleTabClick = (tabId: string) => {
    ...
    navigate(`/chem/${tabId}`, { replace: true });
  ...
  const getCategoryColorClass = (category: string, type = "text") => {
    ...
    >
      ...
      <motion.div className="md:hidden fixed top-20 left-4 z-[60]" whileTap={{ scale: 0.95 }}>
        <Button
          variant="ghost"
          size="icon"
          className={`rounded-full shadow-lg transition-colors duration-200 clay-interactive ${sidebarOpen ? "bg-slate-300/80 dark:bg-slate-700/80 text-slate-800 dark:text-slate-100 hover:bg-slate-400/80 dark:hover:bg-slate-600/80" : "bg-sky-600/80 dark:bg-blue-600/80 text-white hover:bg-sky-700/90 dark:hover:bg-blue-700/90"}`}
      ...
      <div className="relative w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 z-10">
        <div className="w-full lg:w-3/4 mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:gap-x-4 lg:gap-x-6">
            {/* Sidebar */}
            <div
              className={`glass-bold md:w-64 lg:w-72 shrink-0 rounded-xl shadow-lg overflow-hidden
            ...
            >
              <div className="h-full overflow-y-auto">
                {/* View Mode Toggle */}
                <div className="sticky top-0 z-10 p-2 glass-bold border-b border-white/10">
                  <LayoutGroup id="viewModeToggle-chem">
                    <div className="flex p-1 bg-slate-200/50 dark:bg-slate-900/50 rounded-lg">
                      <motion.button
                                  ...
                                  {tabsByCategory[category].map((tab) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`relative w-full flex items-center px-3 py-2 text-sm rounded-md transition-colors group ${isActive ? "text-sky-700 dark:text-white" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-white/10"}`}
                                      >
                                        {isActive && (
                                          <motion.div
                                            className="absolute inset-0 bg-sky-100/80 dark:bg-slate-700/80 rounded-md -z-10"
